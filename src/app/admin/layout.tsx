...
import { LayoutDashboard, Users, Trophy, Flag, Settings, LogOut, FileText, Gavel, ChevronLeft, ShieldCheck, FileSpreadsheet, Lock, Award, Crown } from "lucide-react";
...
const allSidebarLinks = [
  ...
  { name: "Resultados", href: "/admin/resultados", icon: FileSpreadsheet, module: "resultados" },
  { name: "Pilotos & Equipes", href: "/admin/pilotos", icon: Users, module: "pilotos" },
  { name: "Campeonatos", href: "/admin/campeonatos", icon: Trophy, module: "campeonatos" },
  { name: "Corridas", href: "/admin/corridas", icon: Flag, module: "corridas" },
  { name: "Centro de Comissários", href: "/admin/stewards", icon: Gavel, module: "stewards" },
  { name: "Notícias e Regras", href: "/admin/noticias", icon: FileText, module: "noticias" },
  { name: "Gestão de Contas", href: "/admin/contas", icon: Users, module: "contas" },
  { name: "Controle de Acessos", href: "/admin/permissoes", icon: Lock, module: "permissoes" },
  { name: "Gerenciar Conquistas", href: "/admin/conquistas", icon: Award, module: "conquistas" },
  { name: "Hall da Fama", href: "/admin/hall-da-fama", icon: Crown, module: "hall_da_fama" },
  { name: "Configurações", href: "/admin/config", icon: Settings, module: "config" },
];
    ...
    const fetchUserRole = async () => {
      ...
      if (session?.user) {
        ...
        if (fallbackRole === 'superadmin' || fallbackRole === 'admin') {
          setUserPermissions(["dashboard", "resultados", "pilotos", "campeonatos", "corridas", "stewards", "contas", "noticias", "config", "permissoes", "conquistas", "hall_da_fama"]);
    ...
    <div className="flex h-screen bg-background pt-16 mt-[-64px]">
      ...
      <aside className="w-64 border-r border-border/40 bg-card/40 backdrop-blur-md hidden md:flex flex-col h-full overflow-y-auto">
        <div className="p-6 flex flex-col items-center gap-2 text-center mt-4">
          ...
          <h2 className="font-orbitron font-bold text-foreground mt-2 tracking-widest text-sm uppercase">Comando Geral</h2>
          <p className="text-xs text-muted-foreground font-exo2 uppercase">{isMounted ? userRoleName : "carregando..."}</p>
        </div>
        
        <nav className="flex-1 px-4 space-y-1 mt-4">
          {isMounted && allSidebarLinks.filter(link => userPermissions.includes(link.module)).map((link) => {
