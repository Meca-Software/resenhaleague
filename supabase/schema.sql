...
-- 2. SYSTEM USERS (Independent from Supabase Auth)
...
-- 8. STEWARDS (INCIDENTS & PENALTIES)
...
-- 11. INITIAL SUPER ADMIN ACCOUNT & SEED DATA
-- Aqui nós inserimos as contas de teste diretamente na nossa nova tabela customizada de usuários.
-- Obs: 'password_hash' seria uma senha criptografada num cenário real. Aqui usaremos texto puro para demonstração.
...
-- 12. ROW LEVEL SECURITY (RLS) & PUBLIC READ POLICIES
-- Ativa o RLS para todas as tabelas (Proteção contra edição pública)
...
-- Permite leitura pública de todas as tabelas (Qualquer visitante pode ver os dados)
CREATE POLICY "Allow public read access to teams" ON public.teams FOR SELECT USING (true);
CREATE POLICY "Allow public read access to pilots" ON public.pilots FOR SELECT USING (true);
CREATE POLICY "Allow public read access to championships" ON public.championships FOR SELECT USING (true);
...
CREATE POLICY "Allow public read access to qualifying_results" ON public.qualifying_results FOR SELECT USING (true);
CREATE POLICY "Allow public read access to race_results" ON public.race_results FOR SELECT USING (true);
CREATE POLICY "Allow public read access to incidents" ON public.incidents FOR SELECT USING (true);
CREATE POLICY "Allow public read access to penalties" ON public.penalties FOR SELECT USING (true);
CREATE POLICY "Allow public read access to news" ON public.news FOR SELECT USING (true);
CREATE POLICY "Allow public read access to regulations" ON public.regulations FOR SELECT USING (true);
CREATE POLICY "Allow public read access to hall_of_fame" ON public.hall_of_fame FOR SELECT USING (true);
